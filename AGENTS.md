...
| Change | Update |
|---|---|
| Skill added | `README.md` skills table; `CHANGELOG.md` (`Added`); add `registry/discovery.json` and `registry/lifecycle.json` metadata; regenerate the registry; run `npm run validate` and `npm test`. |
| Skill renamed | Directory name and `name` frontmatter together; every internal reference and link; discovery and lifecycle records; `README.md` skills table; `CHANGELOG.md` (`Changed`); regenerate the registry. |
| Skill removed | Delete `skills/<name>/` and its discovery record; move lifecycle metadata to a `removed` tombstone; `README.md` skills table; `CHANGELOG.md` (`Removed`); regenerate the registry. |
| Skill license, status, replacement, origin, or compatibility changes | Update `registry/lifecycle.json`; record a real review event and annual due date where applicable; version an external upstream identity change; regenerate with `--previous` when comparing a base registry. |
